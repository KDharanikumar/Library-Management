		...
		<>
			<div className="container">
				<div className="my-5 text-center">
					<Link to="/portal/add-book" className="sf-button">
						Add Book
					</Link>
				</div>
				{isLoading ? (
										...
										{books.map((book, index) => {
											return (
												<tr>
													...
													<td>
														<button
															disabled={!book.available || cart.some((obj) => obj.id === book.id)}
															onClick={() => {
																addToCart(book);
															}}
													...
													<div>
														<td>
															<Link to={`/portal/library/${book.id}`} className="btn btn-sm btn-warning mx-2">
																View
															</Link>

															<Link to={`/portal/library/edit/${book.id}`} className="btn btn-sm btn-primary mx-2">
																Edit
															</Link>
				...
				{cart.length === 0 ? (
					...
					<>
						<ul className="list-group list-group-numbered  ">
							{cart.map((cartItem) => {
								return <CartList cartItem={cartItem} removeFromCart={removeFromCart}></CartList>;
							})}
						</ul>
					</>
				)}
